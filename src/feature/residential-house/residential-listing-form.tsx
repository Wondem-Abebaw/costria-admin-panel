// components/create/residential/residential-listing-form.tsx
"use client";
...
import { z } from "zod";
import { useResidentialFormStore } from "@/lib/store/residential-form-store";
...
import {
  ...
  type ResidentialListingFormData,
} from "@/lib/validations/residential-listing-schema";
...
export function ResidentialListingForm({
  mode,
  listingId,
  selectedUserId,
}: Props) {
  ...
  const form = useForm<ResidentialListingFormData>({
    resolver: zodResolver(residentialListingSchema),
    defaultValues: {
      title: "",
  ...
  const onSubmit = async (data: ResidentialListingFormData) => {
    if (images.length === 0) {
    ...
    try {
      if (mode === "create") {
        await createAsync({ ...payload, userId: selectedUserId ?? "" });
        toast.success(
          "Your residential property listing has been created successfully!",
        );
      } else {
        await updateAsync({
          id: listingId!,
        ...
        toast.success(
          "Your residential property listing has been updated successfully!",
        );
      }
        ...
        <div className="space-y-4">
          ...
          <div>
            ...
            <Input
              id="title"
              {...register("title")}
              placeholder="e.g., Beautiful 2BR Apartment in Bole"
              className="mt-1"
            />
            {errors.title && (
